...
09_organamnist.py
=================
GEMEX vs GradCAM vs GradCAM++ vs Saliency — OrganAMNIST

Dataset
-------
  Name    : OrganAMNIST
  Task    : abdominal CT organ classification (11 organ types)
  Classes : 11
  Note    : Greyscale 28×28. 11-class — largest class count.
...
Usage
-----
  python 09_organamnist.py
  python 09_organamnist.py --theme light
  python 09_organamnist.py --save-dir ./my_results
  python 09_organamnist.py --n-images 11 --epochs 20 --backend cnn
"""
...
MEDMNIST_DATASETS = {
    'organ': {
        ...
        'classes' : ['Bladder','Femur-L','Femur-R','Heart','Kidney-L',
                     'Kidney-R','Liver','Lung-L','Lung-R','Pancreas','Spleen'],
        'channels': 1,
        'task'    : 'abdominal CT organ classification',
    },
}
# ═══════════════════════════════════════════════════════════════════════
# DATA LOADING
...
def run_dataset(dataset_key, save_dir, n_images, theme, backend, epochs):
    ...
    # ── Evaluate ──────────────────────────────────────────────────────
    x_eval  = X_te_s if backend == 'mlp' else X_te
    ...
    for cls in sorted(selected.keys()):
        ...
        # For RGB, x_inp is flat greyscale (784) for model — consistent
        # with how X_te was constructed (greyscale channel of RGB)
        ...
        pred_cls   = int(model.predict(x_inp.reshape(1,-1))[0])
        ...
        if cnn_net is not None:
            try:
                ...
                x_flat_np = x_raw_i.flatten()
                if n_ch > 1:
                    # RGB: (H*W*C,) → (1,C,H,W)
                    x_cnn = x_flat_np.reshape(1, n_ch, 28, 28).astype(np.float32)
                else:
                    # Greyscale: (H*W,) → (1,1,H,W)
                    x_cnn = x_flat_np.reshape(1, 1, 28, 28).astype(np.float32)
                ...
                c_map   = compute_saliency(model, x_inp, pred_cls).flatten()
                cpp_map = c_map.copy()
        else:
            c_map   = compute_saliency(model, x_inp, pred_cls).flatten()
            cpp_map = c_map.copy()
        s_map = compute_saliency(model, x_inp, pred_cls).flatten()
        ...
        g_del   = deletion_auc(model, x_inp, g_n_px, pred_cls)
        c_del   = deletion_auc(model, x_inp, c_n, pred_cls)
        cpp_del = deletion_auc(model, x_inp, cpp_n, pred_cls)
        s_del   = deletion_auc(model, x_inp, s_n, pred_cls)
...
# ═══════════════════════════════════════════════════════════════════════
# ENTRY POINT — OrganAMNIST
# ═══════════════════════════════════════════════════════════════════════

if __name__ == '__main__':
    import argparse
    parser = argparse.ArgumentParser(
        description='GEMEX vs GradCAM — OrganAMNIST')
    parser.add_argument('--save-dir', default='./organ_results',
                        help='Output directory (default: ./organ_results)')
    parser.add_argument('--n-images', type=int, default=11,
                        help='Number of images to explain (default: 11)')
    ...
    print("  Dataset  : OrganAMNIST")
    print("  Task     : abdominal CT organ classification (11 organ types)")
    print("="*62)
