...
class VizDispatcher:
    """Routes result.plot(kind, theme) calls to the correct function."""

    VALID = ['gsf_bar','beeswarm','force','dependence',
             'attention_heatmap','attention_dwell','attention_vs_effect',
             'bias','network','image_trio','triplet_hypergraph',
             'manifold_surface','timeseries_attribution','all']
    ...
    _MAP = {
        ...
        'attention_heatmap':       _attention_heatmap,
        'attention_dwell':         _attention_dwell,
        'attention_vs_effect':     _attention_vs_effect,
        'bias':                    _bias,
        'network':                 _network,
        'image_trio':              _image_trio,
        'triplet_hypergraph':      _triplet_hypergraph,
        'manifold_surface':        _manifold_surface,
        'timeseries_attribution':  _timeseries_attribution,
    }
    ...
    def plot(self, kind: str = 'gsf_bar', theme: str = 'dark', **kwargs):
        ...
        if kind not in self.VALID:
            raise ValueError(f"Unknown plot kind '{kind}'. Valid: {self.VALID}")
        if kind == 'all':
            return [self.plot(k, theme=theme, **kwargs) for k in self.VALID[:-1]]
        t  = THEMES.get(theme, DARK)
        fn = self._MAP.get(kind)
        if fn is None: return _stub(kind, self.r, t)
        return fn(self.r, t, **kwargs)
